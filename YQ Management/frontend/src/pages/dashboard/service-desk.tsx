...
export default function ServiceDeskToday() {
  ...
  return (
    <AdminLayout pageTitle="Service Desk" noPadding={true}>
      ...
      <div className="flex-1 h-full w-full flex flex-col md:grid md:grid-cols-12 overflow-hidden bg-canvas dark:bg-dark-canvas">
            ...
            {displayPool.length === 0 && !isLoading && (
              <div className="flex flex-col items-center justify-center h-full text-outline animate-in fade-in duration-500 py-16">
                ...
                {visits.length === 0 && tenant?.subdomain && (
                  <a
                    href={getTenantUrl(tenant.subdomain, `/booking${selectedLocationId !== 'all' ? `?locationId=${selectedLocationId}` : ''}`)}
                    target="_blank"

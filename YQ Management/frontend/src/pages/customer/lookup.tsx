...
export default function CustomerLookup() {
  ...
  const handleSearch = async (phoneNumber: string = phone) => {
    ...
    try {
      const data = await fetchApi(`/public-visit/by-phone?phone=${encodeURIComponent(phoneNumber)}`);
      if (!data) throw new Error('Failed to find tickets');
  ...
  const handleCancel = async (accessToken: string) => {
    ...
    try {
      await fetchApi(`/public-visit/${accessToken}/cancel`, {
        method: 'POST',
      });
  ...
  const activeStates = ['WAITING', 'CHECKED_IN', 'IN_SERVICE', 'SCHEDULED', 'CREATED'];
  const activeTickets = visits
    .filter(v => activeStates.includes(v.currentState))
    .sort((a, b) => {
      // Prioritize scheduled time coming up soon
  ...
  const pastTickets = visits.filter(v => !activeStates.includes(v.currentState));
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        {searched && !loading && (
          <div className="space-y-12">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center justify-between mb-6">
                Active Tickets
              ...
              {activeTickets.length === 0 ? (
                ...
                <div className="grid gap-6">
                  {activeTickets.map((visit) => (
                    <TicketCard key={visit.id} visit={visit} getStateColor={getStateColor} handleCancel={handleCancel} />
                  ))}
                </div>
              )}
            </div>
            ...
            {pastTickets.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center justify-between mb-6">
                  Past Tickets
                  <span className="bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-gray-300 text-sm py-1 px-3 rounded-full border border-gray-200 dark:border-zinc-700">
                    {pastTickets.length} found
                  </span>
                </h2>
                <div className="grid gap-6 opacity-75">
                  {pastTickets.map((visit) => (
                    <TicketCard key={visit.id} visit={visit} getStateColor={getStateColor} handleCancel={handleCancel} />
                  ))}
                </div>
              </div>
...
function TicketCard({ visit, getStateColor, handleCancel }: { visit: AnyFixMe, getStateColor: (s: string) => string, handleCancel: (token: string) => void }) {
  const isCancellable = ['WAITING', 'SCHEDULED', 'CREATED'].includes(visit.currentState);
  
  return (
    <div className="bg-white dark:bg-zinc-800 shadow-sm rounded-xl overflow-hidden border border-gray-100 dark:border-zinc-700 hover:shadow-md transition-shadow">
      <div className="p-6">

...
export function QueueHistoryTab({ queueId }: { queueId: string }) {
  const { data: visits = [], isLoading } = useQuery({
    ...
    queryFn: () => fetchApi(`/visit?scope=history&queueId=${queueId}`),
  });
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {completed.map((visit: any) => (
                <tr key={visit.id} className="border-b border-border dark:border-dark-border hover:bg-surface-container-lowest transition-colors">
                  <td className="py-3 px-4 font-medium">{visit.displayId}</td>
                  <td className="py-3 px-4">{visit.customer?.name || 'Unknown'}</td>

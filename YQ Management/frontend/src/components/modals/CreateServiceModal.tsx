...
export function CreateServiceModal({ isOpen, onClose, locationId }: CreateServiceModalProps) {
  ...
  React.useEffect(() => {
    if (locationId) setSelectedLocId(locationId);
  }, [locationId]);
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; description?: string; expectedDuration?: number; locationId?: string }) =>
      fetchApi('/service', { method: 'POST', body: JSON.stringify(data) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['services'] });
      if (locationId) {
        queryClient.invalidateQueries({ queryKey: ['location', locationId, 'services'] });
      }
      toast.success('Service created successfully');
      handleClose();
    },
    onError: () => toast.error('Error creating service'),
  });
  ...
  const handleClose = () => {
    ...
    if (!locationId) setSelectedLocId('');
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {!locationsLoading && locations.length === 0 && (
            <div className="bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 p-4 rounded-xl text-sm border border-amber-200 dark:border-amber-500/20">
              ...
              <a href="/dashboard/settings/resources" className="underline mt-2 inline-block">Go to Settings &gt; Resources to add a location</a>
            </div>
          )}
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-white dark:bg-black/50 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all min-h-[100px]"
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-gray-200 dark:border-white/10">
            ...
            <button 
              type="submit"
              disabled={createMutation.isPending || !name.trim() || locations.length === 0 || (!locationId && !selectedLocId)}
